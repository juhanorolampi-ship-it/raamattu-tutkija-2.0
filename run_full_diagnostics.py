...
def run_diagnostics():
    ...
    alkuperaiset_sanat = sorted(list(set(
        sana for avainsanat in suunnitelma["hakukomennot"].values(
        ) for sana in avainsanat if sana
    )))
    rikastetut_sanat_map = rikasta_avainsanat(
        alkuperaiset_sanat, paivita_token_laskuri)
    end_time = time.perf_counter()
    logging.info(f"Aikaa kului: {end_time - start_time:.2f} sekuntia.")
    ...
    for sana in uniikit_laajennetut_sanat:
        haku_cache[sana] = etsi_ja_laajenna(
    ...
    for i, (osio_nro, alkuperaiset_avainsanat) in enumerate(hakukomennot.items()):
        ...
        for alkup_sana in alkuperaiset_avainsanat:
            laajennetut_sanat = rikastetut_sanat_map.get(
